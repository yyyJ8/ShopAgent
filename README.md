...
| 工具 | 数据源 | 说明 |
|------|--------|------|
| 1 get_products | products 原始表 | 商品主数据，按 SKU/状态/类目/店铺筛选 |
...
| 节点 | 谁做 | 模型 | 说明 |
|------|------|------|------|
| 1 understand | LLM | simple | 意图分类（lookup/anomaly/advice/chat）+ 实体提取 |
| 2 plan | LLM + FC | full + tools | Function Calling 选工具 + 填参数；补调轮会收到缺失数据源提示 |
| 3 call_tools | 代码 | -- | MCP streamable-http 并行执行，单工具失败不崩；补调时去重 |
| 4 data_check | 代码 | -- | 检查 anomaly_rules 所需数据源覆盖，缺失 -> 回 plan，>=2 轮熔断降级 |
| 5 analyze | LLM | full | 数据解读 + 交叉验证 + 派生数据可靠性警告 |
| 6 detect | 代码 + LLM | full | 规则扫描（6 条阈值规则 x 多数据源合并 + 聚合 + 比例重算）+ LLM 归因 |
...
| 总分 | **11.5/12.5 (92%)** |
| 意图分类 | 10/10 (100%) |
| 工具选择 | 8/10 (80%) - 2 次 LLM 选原始表替代派生表，符合设计策略 |
| 异常命中 | 5/6 规则生效，全扫描 63 条异常 |
| 数据完整性环 | 触发补调 1 次，零失误 |
...
| 阶段 | 内容 | 状态 |
|------|------|------|
| Phase 1 MCP | 8 个语义化数据工具 + store_id 过滤 + 3 种传输模式 | done |
| Phase 1 Agent | 8 节点全框架 + 数据完整性环 + 6 条规则扫描 + Function Calling | done |
| Phase 1 评测 | 10 题评测闭环，92% 准确率 | done |
| Phase 2 | Streamlit UI + 多轮对话 + 设计文档 | todo |
